...
void I_InitGraphics (void)
{
    ...
    /* ESP32-C6 PORT: pin the framebuffer in .bss instead of malloc'ing.
     * We only have ~70 KiB of free heap after IDF init, the largest
     * contiguous block is ~51 KiB, and the zone has already eaten the
     * biggest fragment by the time we get here — so a 64 KiB malloc
     * reliably fails. Putting it in .bss reserves it at link time
     * (link-time guarantee > runtime fragmentation). */
    static byte s_video_buffer[SCREENWIDTH * SCREENHEIGHT];
    I_VideoBuffer = s_video_buffer;

    /* doomgeneric_Create() up in doomgeneric.c does
     *     DG_ScreenBuffer = malloc(RESX * RESY * 4);  // 256 KiB
     * which silently returns NULL on this badge. The vanilla
     * I_FinishUpdate then memcpy's into NULL and crashes.
     *
     * In CMAP256 mode pixel_t is one byte and DG_ScreenBuffer holds
     * exactly the same 320*200 palette indices as I_VideoBuffer, so
     * we just alias them. The per-line memcpy in I_FinishUpdate
     * becomes a self-copy (harmless, ROM memcpy handles src==dst).
     * Saves the 256 KiB allocation we never had room for. */
...
{
    /* I_VideoBuffer points to a static .bss array now; nothing to free. */
    I_VideoBuffer = NULL;
}
