...
// ESP32-C6 PORT: this is a per-level stats history retained only when
// the `-statdump` cmdline arg is set. We never pass that arg, so the
// 6.4 KB array (32 × ~200 B) is dead weight. Cut to 1 entry; the
// StatCopy gate already checks num_captured_stats < MAX_CAPTURES so
// after the first call it'll just no-op.
