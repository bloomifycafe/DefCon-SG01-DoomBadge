...
void P_LoadBlockMap (int lump)
{
    ...
    // ESP32-C6 PORT: the block map is read-only and stored as LE shorts
    // in the WAD. RISC-V is LE, so SHORT() is identity and the in-place
    // byte-swap loop is dead code. Point blockmaplump directly at the
    // flash-mapped lump and skip both the Z_Malloc and the swap. Saves
    // ~8 KB of zone per level — critical when the zone is 54 KiB total.
    // (W_CacheLumpNum returns a const flash pointer for mmap'd lumps;
    // we cast away const because DOOM's signature is non-const, but
    // we never write through it on this path.)
