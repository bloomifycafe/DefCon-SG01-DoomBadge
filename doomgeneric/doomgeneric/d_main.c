...
void D_Display (void)
{
    ...
    // ESP32-C6 PORT: the screen-melt transition effect needs TWO 64 KiB
    // full-frame capture buffers (start + end) plus a wipe-state buffer.
    // We can't afford 130 KiB of zone for a cosmetic transition, so we
    // disable wipes entirely — gamestate just snaps from menu to level.
    // The game otherwise behaves identically; only the visual flourish
    // is gone.
    wipe = false;
    if (gamestate != wipegamestate) {
        /* still update wipegamestate so subsequent frames don't keep
         * trying to wipe — without this we'd skip wipe alloc but flag
...
void D_DoAdvanceDemo (void)
{
	...
	break;
      case 1:
	/* ESP32-C6 PORT: skip demo playback. The shareware demos play E1M5
 * which needs ~159 KiB of zone for its level geometry — way past our
 * 126 KiB zone. Falling through ticks the demosequence forward to the
 * next static page (TITLEPIC / CREDIT / HELP2) so the title cycle
 * keeps going without crashing. */ advancedemo = true;
	break;
      case 2:
	pagetic = 200;
	gamestate = GS_DEMOSCREEN;
	...
	break;
      case 3:
	advancedemo = true; /* skip demo — see demo1 case above */
	break;
      case 4:
	gamestate = GS_DEMOSCREEN;
	if ( gamemode == commercial)
...
void D_IdentifyVersion(void)
{
    ...
    if (gamemission == none)
    {
        ...
        {
            if (!strncasecmp(lumpinfo_name(&lumpinfo[i]), "MAP01", 8))
            {
                gamemission = doom2;
                break;
            }
            else if (!strncasecmp(lumpinfo_name(&lumpinfo[i]), "E1M1", 8))
            {
                gamemission = doom;
                break;
            }
...
//    W_PrintDirectory();
    ...
    if (p)
    {
        ...
        if (D_AddFile(file))
        {
            M_StringCopy(demolumpname,
                         lumpinfo_name(&lumpinfo[numlumps - 1]),
                         sizeof(demolumpname));
        }
        else
        {
    ...
    if (M_ParmExists("-dehlump"))
    {
        ...
        {
            if (!strncmp(lumpinfo_name(&lumpinfo[i]), "DEHACKED", 8))
            {
