...
static void ChecksumAddLump(sha1_context_t *sha1_context, lumpinfo_t *lump)
{
    ...
    /* ESP32-C6 PORT: lumpinfo_t no longer holds name/position/size/
     * wad_file directly; pull them via accessors that read from the
     * flash WAD directory. wad_file is always our single global. */
    M_StringCopy(buf, lumpinfo_name(lump), sizeof(buf));
    SHA1_UpdateString(sha1_context, buf);
    SHA1_UpdateInt32(sha1_context, GetFileNumber(wad_file_global));
    SHA1_UpdateInt32(sha1_context, lumpinfo_position(lump));
    SHA1_UpdateInt32(sha1_context, lumpinfo_size(lump));
}
